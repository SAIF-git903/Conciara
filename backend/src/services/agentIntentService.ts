...
/**
 * Classify user message intent. Order matters: conversational intents are
 * checked first so short acknowledgments are not misclassified as queries.
 */
...
/** Lightweight session state derived from conversation history (no DB). */
...
/**
 * Derive session state from recent conversation. Used to tailor system prompt
 * (e.g. "last product mentioned" for contextual goodbyes).
 */
