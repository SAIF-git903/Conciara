 ...
 * Filters by website IDs if provided (for domain-based access control)
 */
export async function getAllConversations(
  ...
  treeId?: number,
  userId?: string,
  websiteIds?: number[] | null // null = all websites (admin), undefined = no filter, array = specific websites
  ...
  try {
    let query = `
      ...
      LEFT JOIN dialog_trees dt ON cs.tree_id = dt.id
    ...
    // Filter by website IDs if provided (for editors)
    if (websiteIds !== null && websiteIds !== undefined && websiteIds.length > 0) {
      const placeholders = websiteIds.map((_, i) => `$${paramIndex + i}`).join(', ');
      conditions.push(`w.id IN (${placeholders})`);
      params.push(...websiteIds);
      paramIndex += websiteIds.length;
    }
    ...
    query += `
      GROUP BY cs.session_id, cs.tree_id, cs.user_id, cs.created_at, cs.updated_at, w.id, w.name, w.domain
