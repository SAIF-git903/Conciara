...
function extractWhatsAppLinksFromText(text: string): string[] {
  const keywords = /whatsapp|whats\s*app|\bwa\b|chat\s+with\s+us|contact\s+us\s+on/i;
  const phoneLike = /[\+]?[\d\s\-\.\(\)]{10,20}/g;
  const links: string[] = [];
  const seen = new Set<string>();
  let m: RegExpExecArray | null;
  // Match up to 60 chars before keyword and 60 after so we catch numbers on either side
...
function extractFromHtml(
  ...
  // Remove script/style for content
  $('script, style').remove();
  ...
  // Fallback: any link with /product/ in path
  ...
  // Support links: help, support, faq, contact
  ...
  // WhatsApp: existing wa.me / api.whatsapp.com links
  ...
  // WhatsApp: phone numbers near keywords in body text
...
export async function crawlWebsite(
  ...
  // 2) Get links from seed (reuse seedHtml from step 1)
  ...
  // 3) Crawl queue with concurrency
  const queue = [...toCrawl];
