...
export async function retrieveChunks(
  ...
  if (hasVector && queryEmbedding) {
    ...
    return (rows.rows as { id: number; content: string; document_id: number; chunk_index: number; similarity?: number }[])
  ...
  if (words.length === 0) {
    ...
    return (rows.rows as { id: number; content: string; document_id: number; chunk_index: number }[]).map((r) => ({
      id: r.id,
      content: r.content,
      documentId: r.document_id,
      chunkIndex: r.chunk_index,
