...
/** Tool call from the model (for custom API actions). */
...
/** Result of a completion that may include tool calls. */
...
/**
 * Agent chat with optional tools (function calling).
 * Returns the message content and any tool_calls so the caller can execute and re-call.
 */
/** Tool definition for OpenAI (function schema). */
...
export async function chatCompletionWithTools(
  ...
  if (!openai || !apiKey) {
    return {
      content: "I'm not configured to respond yet. Please add an API key for the chat model.",
      toolCalls: null,
    };
  }
  ...
  const openaiMessages: Array<
    { role: 'system'; content: string } | { role: 'user'; content: string } | { role: 'assistant'; content: string; tool_calls?: Array<{ id: string; type: 'function'; function: { name: string; arguments: string } }> } | { role: 'tool'; content: string; tool_call_id: string }
  > = [
    { role: 'system', content: systemContent || 'You are a helpful assistant.' },
    ...messages.map((m) => {
      if (m.role === 'user') {
        return { role: 'user' as const, content: (m as { content: string }).content };
      }
      if (m.role === 'tool') {
        return {
          role: 'tool' as const,
          content: (m as { content: string; toolCallId: string }).content,
          tool_call_id: (m as { toolCallId: string }).toolCallId,
        };
      }
      const a = m as { role: 'assistant'; content: string | null; toolCalls?: ToolCall[] };
      if (a.toolCalls?.length) {
        return {
          role: 'assistant' as const,
          content: a.content ?? '',
          tool_calls: a.toolCalls.map((tc) => ({
            id: tc.id,
      ...
      return { role: 'assistant' as const, content: a.content ?? '' };
    }),
  ];
  ...
  const response = await openai.chat.completions.create({
    ...
    tools: tools.length > 0 ? (tools as Parameters<typeof openai.chat.completions.create>[0]['tools']) : undefined,
    max_tokens: maxTokens,
    temperature,
  });
  ...
  if (!msg) {
    return { content: "I couldn't generate a response.", toolCalls: null };
  }
