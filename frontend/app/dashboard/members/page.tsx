...
export default function MembersPage() {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    setInviting(true)
    try {
      const { data } = await api.post<{ member?: Member; pendingInvite?: boolean; inviteLink?: string; expiresAt?: string }>(
        `/workspaces/${currentWorkspace.id}/members`,
  ...
  const handleResendInvite = async (email: string) => {
    ...
    setError(null)
    try {
      await api.post<{ inviteLink: string; expiresAt: string }>(
        `/workspaces/${currentWorkspace.id}/invites/resend`,
        { email }
      )
      await fetchMembers()
    } catch (e: unknown) {
  ...
  const handleRemove = async () => {
    ...
    try {
      await api.delete(
        `/workspaces/${currentWorkspace.id}/members/${removeTarget.userId}`
      )
