...
import { getOnboardingWorkspaceId, getOnboardingAgentId, getOnboardingLinkDone } from '@/lib/onboarding'

const steps = [
  { path: '/onboarding/workspace', label: 'Workspace', short: 'Workspace', Icon: LayoutDashboard },
  { path: '/onboarding/link', label: 'Website', short: 'Link', Icon: Link2 },
  { path: '/onboarding/configure', label: 'Chatbot', short: 'Configure', Icon: Settings },
  { path: '/onboarding/personality', label: 'Personality', short: 'Agent', Icon: Bot },
]
...
export default function OnboardingLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (pathname === '/onboarding/workspace' && wid != null) {
      router.replace('/onboarding/link')
      return
    }
    if (pathname === '/onboarding/link') {
      if (wid == null) {
    ...
    if (pathname === '/onboarding/configure') {
      if (wid == null) {
        router.replace('/onboarding/workspace')
        return
      }
    ...
    if (pathname === '/onboarding/personality') {
      if (wid == null) {
        router.replace('/onboarding/workspace')
        return
      }
  ...
  const getStepHref = (i: number): string | null => {
    const step = steps[i]
    if (i === 0) {
      return workspaceId != null ? '/onboarding/link' : step.path
    }
