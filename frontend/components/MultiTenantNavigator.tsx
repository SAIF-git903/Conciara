...
import { userApi } from '@/lib/authApi'
...
export default function MultiTenantNavigator({ onSelectTree, selectedTreeId, onWebsiteChange }: MultiTenantNavigatorProps) {
  ...
  const [allWebsites, setAllWebsites] = useState<Website[]>([]) // All websites (for filtering)
  const [websites, setWebsites] = useState<Website[]>([]) // Filtered websites
  const [skins, setSkins] = useState<Skin[]>([])
  const [variations, setVariations] = useState<ABVariation[]>([])
  const [trees, setTrees] = useState<DialogTree[]>([])
  const [allUsers, setAllUsers] = useState<Array<{ id: number; email: string; fullName?: string; role?: string }>>([]) // For manager assignment
  ...
  const [formData, setFormData] = useState<{ name: string; description: string; domain: string; theme_config: SkinConfig | null; is_active?: boolean; editor_user_id?: number }>({ name: '', description: '', domain: '', theme_config: null })
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      if (isManager || isViewer) {
        // For managers and viewers, load their assigned websites directly (no customer types)
        await loadManagerWebsites(abortController.signal)
      } else {
        // For admins only, load customer types first
        await loadCustomerTypes(abortController.signal)
  ...
  useEffect(() => {
    // For managers and viewers, skip customer type selection - websites are loaded directly
    if (isManager || isViewer) return
    ...
    if (selectedCustomerType && isMounted) {
      loadWebsites(selectedCustomerType.id, abortController.signal)
    } else {
      setWebsites([])
      setSkins([])
      setVariations([])
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedCustomerType, isManager, isViewer])
  ...
  const loadCustomerTypes = async (signal?: AbortSignal) => {
    try {
      ...
      if (data.length > 0 && !selectedCustomerType && isMountedRef.current) {
  ...
  const loadAllUsers = async () => {
    try {
      ...
      setAllUsers(users.filter(u => u.role === 'manager' || u.role === 'viewer').map(u => ({
        id: u.id,
        email: u.email,
        fullName: u.fullName,
        role: u.role
  ...
  // Load websites directly for managers (without customer type selection)
  const loadManagerWebsites = async (signal?: AbortSignal) => {
    try {
      if (!isMountedRef.current || !user?.websites || user.websites.length === 0) return
      
      setLoading(true)
      
      // Get website IDs assigned to manager
      const allowedWebsiteIds = user.websites.map(w => w.websiteId)
      
      // Fetch websites directly by IDs (no customer type API call needed)
      const websitesData = await websiteApi.getByIds(allowedWebsiteIds)
      if (signal?.aborted || !isMountedRef.current) return
      ...
      if (websitesData.length > 0 && !selectedWebsite && isMountedRef.current) {
  ...
  const loadWebsites = async (customerTypeId: number, signal?: AbortSignal) => {
    try {
      if (!isMountedRef.current) return
      setLoading(true)
      ...
      // Filter websites based on user's assigned domains (if not admin)
      let filteredData = data
      if (!isAdmin && user?.websites && user.websites.length > 0) {
        const allowedWebsiteIds = user.websites.map(w => w.websiteId)
        filteredData = data.filter(w => allowedWebsiteIds.includes(w.id))
      }
      
      if (!isMountedRef.current) return
      setWebsites(filteredData)
      if (filteredData.length > 0 && !selectedWebsite && isMountedRef.current) {
        setSelectedWebsite(filteredData[0])
      } else if (!filteredData.find(w => w.id === selectedWebsite?.id) && isMountedRef.current) {
        setSelectedWebsite(null)
      }
    } catch (err: any) {
      if (signal?.aborted || !isMountedRef.current) return
  ...
  const loadSkins = async (websiteId: number) => {
    try {
      ...
      if (!isMountedRef.current) return
      setSkins(data)
      if (data.length > 0 && !selectedSkin && isMountedRef.current) {
  ...
  const loadVariations = async (skinId: number) => {
    try {
      ...
      if (!isMountedRef.current) return
      setVariations(data)
      if (data.length > 0 && !selectedVariation && isMountedRef.current) {
      ...
      switch (showCreateModal.type) {
        ...
        case 'website':
          ...
          if (showCreateModal.isEdit && showCreateModal.data) {
            ...
            // Create mode
            // Create website with optional manager assignment
            ...
            // Add manager assignment if provided and user is admin
            if (isAdmin && formData.editor_user_id) {
              createData.editor_user_id = formData.editor_user_id
            }
            
            newItem = await websiteApi.create(
              createData.customer_type_id,
              createData.name,
              createData.description,
              createData.domain,
              createData.editor_user_id
            )
  ...
  return (
    <div className="h-full flex flex-col bg-white border-r border-gray-200">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {/* Customer Types - Hidden for managers and viewers */}
        {!isManager && !isViewer && (
          <div>
        ...
        {/* Websites - Show directly for managers/viewers, or after customer type selection for admins */}
        {(((isManager || isViewer) && websites.length > 0) || (!isManager && !isViewer && selectedCustomerType)) && (
          <div>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-xs font-semibold text-gray-700 uppercase tracking-wide flex items-center gap-1">
                {!isManager && !isViewer && <ChevronRight className="w-3 h-3" />}
                {(isManager || isViewer) ? 'Your Domains' : 'Websites'}
              </h3>
              {!isViewer && (
                <button
              ...
              <div className="space-y-4">
                ...
                {showCreateModal.type === 'website' && (
                  <>
                    ...
                    {!showCreateModal.isEdit && isAdmin && allUsers.length > 0 && (
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                          Assign Manager (Optional)
                          <span className="block text-xs text-gray-500 font-normal mt-1">
                            Select a manager to assign to this domain
                          </span>
                        </label>
                        <select
                          value={formData.editor_user_id || ''}
                          onChange={(e) => setFormData({ ...formData, editor_user_id: e.target.value ? parseInt(e.target.value) : undefined })}
                          className="w-full p-2.5 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                        >
